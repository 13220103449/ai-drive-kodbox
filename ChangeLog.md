## 1.69 版本更新 
> **主要新增**: 细节优化,安全优化 ...  
> **1.69.01 发布于: 2026.8.17**  
> **1.69.03 发布于: 2026.8.21**  

- 文件管理
	- 文档变更,更新用户或部门空间占用大小计算优化(上传/新建/编辑/删除等操作),优化sql性能;
	- 搜索勾选搜索内容时文案优化: 全文搜索:支持搜索office,pdf等文件; aiRag:支持智能语义搜索 (RAG);
	- 自动定时清空系统回收站时,默认不删除为大小0的文件夹,或空文件夹;
	- 轻应用开关: 后台开关,不存在配置/默认关闭, 关闭时桌面/新建不显示轻应用入口;
	- 搜索框placeholder: 全文搜索:text,office,pdf...; aiRag:智能搜索,搜索图标区分;
	- 路由url变更默认使用pushState方式,兼容iOS 27等浏览器(location.href 初始打开url包含hash时,更新hash导致页面重新刷新);
	- 外链分享分享目录,上层文件夹设置了密码时忽略密码检测(从所在文件夹到分享跟文件夹直接的做检测);
	- 内部协作文件夹,子目录设置了密码时,其他只读用户读写时密码校验优化;
	- 文件历史版本优化: 有编辑权限才进入显示; 有编辑权限但没有下载权限时禁用下载(前端入口屏蔽,后端接口拦截);
	- 文件夹压缩默认限制大小24GB(按teamos硬件估算);
- 插件优化
	- 项目插件: 彻底删除后,自动移除项目文件夹协作内容(与我协作,系统内容);
	- 图片预览插件更新, 点击图片播放动画过渡处理优化(图片为 object-fit:cover 时初始位置及大小优化);
	- 全文索引插件处理优化;
	- 插件获取系统临时目录,自动创建后缓存优化,避免频繁db查询;
	- 内网穿透,office代理配置二级域名去除以下划线开头(部分手机浏览器不支持);自定义端口/二级目录office地址自动跟随网盘
	- cockpit硬件模块插件优化: 页面样式载入优化;迁移存储拦截调整;
	- AI助手插件,多模态生成支持及完善(多模态全模型调用支持)
		- asr 语音转文本, 占位符更新;
		- audio 文本转语音: 配置,音色选择; 自定义音色文件;音色选择UI优化;
		- image 图片生成: 文生图,图生图,图片编辑;
		- video 视频生成: 文生视频,图生视频,首尾帧生视频,视频编辑;
		- 3d 3D生成: 文生3D,图生3D;
		- 通用配置表;后端配置解析; 配置表前端解析; 通用请求model处理(请求,参数处理,模版解析); 对话使用;
		- 接口自适应: 模型支持检测,接口调用自适应(eg: 火山豆包图片编辑接口); 配置参数UI交互;
		- 多模态生成入口处理: 自适应显示支持的模态生成; 切换处理; 图标显示处理;对话创建自动处理;
		- 多模态生成控制器处理; 入口控制器自适应切换,自动跟随;自适应对话入口; 首次创建会话加入对话类型,后续切换会话自适应生成入口;
		- 多模态生成请求前端处理: 请求接口重定向处理,请求参数构造处理(不再处理上下文对话;工具等等);
		- 多模态生成后端处理: 请求代理转发(参数处理),结束后自动创建会话消息,前端展示处理; 
		- 生成请求手动停止处理(默认请求后自动创建对话回复; 手动中断后不再响应)
		- 重新生成处理(请求参数处理); 请求创建后,包含文件时处理检测可读权限; 构造请求url;
		- 多模态生成结果展示; 音频/视频/图片/文字等独立展示; 图片生成展示优化;
		- 音频播放器优化: 播放seekbar显示,更随播放进度,拖拽时更随;初始显示默认波形背景图,文件加载后不再显示;
		- 3d文件生成, 兼容火山请求后压缩包,glb文件自动提取处理;
		- 视频生成内容,显示优化,直接播放处理(依赖 Dplayer 播放器插件;没有则忽略渲染);
		- 多模态生成,传入文件列表时,文件信息自动加入外链url处理,便于后续外链分享对话展示(权限处理);
		- 请求loading处理; 请求后内容展示(图片,音频,视频,3d文件); 统一loading展示及交互;进度展示处理,剩余时间预估显示;
		- 生成任务异步请求: 首次创建临时消息,刷新后自动获取任务状态(未完成则构造loading,自动检测进度),完成更新消息内容;出错后移除临时消息;
		- 异步请求页(新消息/重新生成/切换新话题): 正常完成/请求出错/后台任务终止/手动中断; 代码异常或进程终止(需手动停止请求,超时检测);
		- 异步刷新页(新消息/重新生成/切换新话题): 正常完成/请求出错/后台任务终止/手动中断; 代码异常或进程终止(需手动停止请求,超时检测);
		- 异步任务异常处理: 任务前/任务中/任务后, 系统报错/缓存失效/被删除, 自动清理临时消息/自动超时; 生成完成加入彩纸效果;
		- 生成配置UI配置: 占位符,ui文字信息, 字段文案及icon; 显示字段;
		- 生成配置文件选择: 构造文件选择(单选,多选; 文件类型);配置表表单项构造: 字段对应到form; 选择数值筛选处理; 请求参数统一处理;
		- 表单优化: 分段选择显示优化; 表单构造字段顺序按配置定义顺序; 音色字段--支持分组筛选处理;
		- 快捷选择表单,字段处理(多个字段每个独占; 选中后关闭弹出层; 值更新显示配置值--同显示字段);
		- 文件选择处理; 单个文件,多个文件自动处理; 选择后更新显示处理; 修改/移除选择文件处理; title显示; 必填项标记;
		- 选择文件;打开支持,图片打开多张自动切换处理支持; 文件右键菜单支持;文件拖放支持,根据扩展名判断是否允许拖入;
		- 表单结果处理: 获取结果,统一值处理; 设置配置; 检测是否可提交处理,界面更新;
		- 模型切换,自动载入之前配置,配置变更本地记住;模型切换文件自适应加入; 根据允许的文件扩展名自动设置;
		- 编辑用户消息请求参数处理,自动设置提示词/文件/配置到当前对话(复用参数/文件)
		- 重新生成请求参数处理: 使用当前模型参数; 复用提示词/文件选择;
		- 消息--配置参数显示(对话/收藏/外链分享/后台消息查看); 
		- AI多模态生成,文件生成加入所在对话位置 描述: aiDoc-$topicID-$chatID;meta:aiDocTopicID/aiDocChatID
		- 文本转语音,结果数据加入词位置解析结果(支持的模型处理,fun-asr);
		- 文本转语音支持词位置的,文本展示优化, 进度自动跟随(播放器时间更新,自动高亮对应文本); 词点击自动跳转到播放器对应位置;性能优化处理;
		- 视频缩略图,外链分享缩略图显示优化;
		- AI生成-工具加入(文生图,图片生成,3d生成,文本转语音,语音转文本); 工具自动加入,模型表单映射处理为tools参数定义; 模型未配置则忽略;后台开关,模型配置;
		- AI生成-工具调用结果解析, 文件内容展示; 多模态生成工具调用链路优化;工具调用处理;进度更新,切换对话展示;文件生成内容展示(dom不更新);
		- minimax H3 视频生成兼容支持;(官方接口; OpenRouter 调用兼容);
		- 细节优化: 生成过程中切换话题,再切换回时进度保持(chat=>自动生成; 多模态=> 进度更新); 多模态生成--刷新页停止生成,位置自动保持处理;
		- 其他优化: 多语言处理; 深色浅色UI自适应处理;
	- AI助手插件其他优化:
		- 模型关闭或不存在时历史对话展示优化(可用模型为空;禁止发送/重新生成/生成类型模型为空情况);
		- tools工具调用过程参数简化; toolRun后端检测检测topicID归属,校验用量限制情况;
		- 切换会话滚动到底部优化(切换会话, markdown渲染完成,文件播放加载完成); 输入框粘贴内容自动滚动到最后; 流式生成自动滚动到最后位置优化;
		- 管理员时--模型选择最后加入--管理模型入口;
		- 对话列表绘制优化, 子模版处理,多模块拆分复用优化;
		- 对话渲染完成统一方法调用 chatRenderAfter (mermaid渲染等情况处理);
		- 工具调用文件生成后,后续对话携带新建文件信息,便于基于文件继续操作;
		- 工具调用优化: 搜索/文件列表/rag搜索--结果增加分页信息,请求参数加入分页参数; 
		- 模型兼容优化: qwen3.6-35b等部分模型工具调用结构解析兼容; tool调用目录参数多增加一层目录名情况兼容支持;
		- 联网搜索结果整理优化: 网页转markdown内容优化(数据清洗),去除无关内容; 搜索,清洗,提取配置表处理;
		- 上下文超出长度,自动截取优化(自动截取会话文本--保留最短长度处理; 工具调用截断时置空,调用与调用结果成对出现,移除时成对移除);
		- URL路由优化: 新对话不同tab配置URL; 后台管理页面tab切换加入;
		- 细节优化: 提交后自动清理当前文件; token消耗数量显示优化(中文按万区分);
	- 全文检索插件增加详细的转换失败消息日志;
	- 客户端插件优化: 页面重构;适配功能更新;增加多版本支持; 界面优化调整
	- 其他插件优化: 第三方同步插件部门/用户管理,禁用部门失效调整;统一认证插件稳定性优化;缩略图插件并发限制优化;登录二次验证增加验证器方式;
- 其他优化:
	- 后台管理细节优化: 菜单管理系统默认项编辑异常调整;概览查询优化,增加缓存等; 
	- 系统设置默认仅树目录罗列;
	- 通知中心插件优化调整:事件等级调整;备份异常消息调;并发覆盖异常修复,存储异常修复后通知未停止发送问题修复(存储列表缓存未更新);
	- repair增加 根据{io:x}路径清除物理文件不存在于io_file中的接口；其他接口path条件查询优化(改条件拼接为根据id列表分块查询);
	- formMake表单增强: radio/checkbox 等单选/多选 支持分组筛选; select 结果显示高度自适应处理;
	- hook动作触发,循环绑定情况校验处理;避免死循环情况, 环路情况,多条子环路情况检测(递归深度最大为30层);
	- 刷新页面自动清理临时文件(tmp/magick-*,call方法);
	- 弹窗组件优化, 当开启了lock蒙层时最小化显示优化(自动显示或隐藏蒙层);
	- 插件情况--打开文件选择/进入文件夹, 默认不配置标签(避免最小化不可见问题);
	- markdown渲染优化: 支持 --- Front Matter 语法(前置元数据); 编辑时性能优化; 相对路径图片显示异常问题优化;
	- 自定义菜单,添加的外部链接嵌入页面时, mapRouter页面url优化,刷新后能继续保持;
	
##### bug fixed
- 内部协作分享,文件夹复制到其他存储时子内容文件名异常情况修复;


## 1.68 版本更新 
> **主要新增**: 细节优化,安全优化 ...  
> **1.68.01 发布于: 2026.6.22**  
> **1.68.10 发布于: 2026.7.1**  

- 文件管理
	- 文件列表支持隐藏指定扩展名文件(后台配置--列表目录排除: 增加形如"*.temp,*.tmp");
	- 外链分享支持编辑权限时,操作记录,修改者/创建者用户信息处理优化(统一为游客;区分系统操作; USER_ID 统一替换为KodUser::id());
	- 复制/移动/从回收站还原, 操作成功后tips显示文件列表(可点击,点击进入文档目录并选中);
	- 回收站还原优化: 父子层级同时存, 选择上层/子层级还原时分别处理优化; 还原上层,回收站中子层内容继续保持在回收站;
	- 系统回收站优化: 清空或彻底删除回收站内容时,移到系统回收站保留删除前目录结构(父子目录同时存在时,避免子目录被移动到了用户根目录); 
	- 直接彻底删除,清空到系统回收站优化(同时选中父子文件夹删除的情况);
	- 自动定时清空系统回收站时,默认不删除为大小0的文件夹,或空文件夹;
	- webdav 插件优化: 客户端删除文件后,再获取文件时不默认重命名;
	- 其他优化: 载入文件列表时缩略图插件临时目录生成调用优化;
- 插件优化
	- 项目管理插件:日历更新, 拖拽日历提示tips跟随处理优化;
	- AI助手插件: 模型服务统一更新(openrouter支持显示价格); 大模型请求通用配置表处理; 多模态生成后端支持完善;诸多细节优化;
	- oemCockpit: 脚本更新;raid创建失败消息提醒调整;
	- 百度网盘插件:存储挂载driver优化,文件文件夹移动/复制/重命名,返回路径(返回true导致新建文件1);新建文件其他地方存在同名文件时异常情况;
	- 其他优化: onlyoffice去掉pdf格式支持;全文检索插件php兼容调整
- 其他优化:
	- 数据库,redis等配置连接异常时,页面报错处理(避免白屏loading);
	- 用户批量导入优化: 新增自定义字段导入支持, 支持按自定义字段去重支持;
	- 安全优化(未登录,部分服务器信息屏蔽输出);
	- 后台管理: 数据库切换,备份优化,数据库切换任务获取时长调整; 文件备份增加时间范围设置功能;存储导入增加数量检测;
	- 安装页面添加多语言切换;
	- markdown 渲染mermaid图表,AI生成部分语法错误时的兼容处理优化(mindmap :::占位符优化; timeline key/value中冒号转义处理);
	- 存储自动检测优化(当File表中没有该存储文件时不检测;checkStoreList;StorageModel->checkConfig);
	- 内置库优化: _get方法支持[]数组嵌套合并处理;
	- formMaker 表单tab菜单通用默认记住最后打开的位置(再次打开表单自动定位上次位置);
	- php 版本兼容; 插件中代码异常时,js输出报错处理优化;
	- 其他优化: office阅读器提示及编辑按钮优化;分享页文件强制以对话框形式打开,避免新页面被拦截;
	- 细节优化: formMaker表单,子表单table显示列拖拽显示内容优化,内容超出滚动条优化; 分享图标标签优化;
	
##### bug fixed
- 删除到系统回收站时,文件夹重复创建问题优化;



## 1.67 版本更新 
> **主要新增**: 细节优化 ...  
> **1.67.01 发布于: 2026.5.11**  
> **1.67.02 发布于: 2026.5.13**  

- 文件管理
	- 文件夹下内容超多情况,复制时内存占用处理优化(超过20万文件; 分批处理,避免内存占满); 获取文件名是否存在和自动重命名获取优化;
	- 上传到临时目录优化, 获取上传目标文件夹所在挂载磁盘的根目录,创建tmp临时文件夹作为中转(统一位置);
	- 系统回收站删除大数据目录兼容优化(超过20万子文件情况);
	- webdav 插件优化,挂载根目录时,自适应当前用户设置的多语言;
	- 其他细节优化: 上传对话框icon显示优化; 对话框最小化不可点处理; 用户选择用户头像显示优化;
- 插件优化
	- 部分插件多语言更新;
	- onlyoffice优化: h5页面屏蔽回退菜单;外链分享下载异常调整-path权限;
	- AI助手插件优化:
		- AI模型服务平台,获取模型列表自动匹配信息优化; 配置归类处理;模型配置更新;
		- 插件加载优化;插件配置载入模型列表,忽略网络请求(以及代理请求时临时token获取);避免没有网络或者请求慢时卡住首页加载;
		- AI问答界面, 工具调用中状态处理优化;
		- DeepSeek V4相关模型属性加入,快捷添加模型服务中的内置模型优化; 问答toolCall兼容(回答加入reasoning_content);
		- OCR解析文件入库优化, pdf转图片命令行处理优化(处理为异步,动态输出保持不中断), 错误处理优化;
		- OCR 服务平台解析兼容: OCR解析支持api平台判断处理优化; 百度云平台 PaddleOCR接入调用支持; 经过ocr处理过的文档展示区分;
		- ocr处理pdf文件筛选优化,大于100页时忽略;
		- AI回答交互优化, 让用户选择时题目分组识别优化;
		- 外链分享智能体,会话上限自动处理;
		- 延迟加载模型列表时,首次加载后工具状态更新优化, 默认初始选择文件加入处理;
		- 对话选择的ai模型, 延迟加载模型服务后自动更新;
		- AI输出流式渲染,高度自适应处理,兼容mermaid图表绘制后高度变小情况界面优化;
		- 对话提交,模型信息保存, 方便模型卸载禁用等情况显示细节;
		- 系统提示词优化: 文件夹搜索,rag搜索处理优化;
		- 当调用搜索文件夹工具,搜索根部门时,加入子部门文件搜索;
		- 细节优化: 后台管理--模型列表--移动端显示优化; 移动端模型服务的模型拖拽优化; 外链分享智能体页面显示优化;外链分享AI对话页面显示优化;
		- 细节优化: 模型服务列表,修改模型时自动去除不包含的模型(已下架等情况);最大输出max_token不再设置上限;
		- 细节优化: 智能体编辑,内置对话组,ai/user内容允许为空,为空时不显示; 对话输出时光标显示位置优化(最后一个文本节点后,避免空节点情况);
	- 硬件内置插件: oemCockpit适配v6调整;增加多语言自动切换,优化登录;加入ai输出提示;
	- 全文检索插件增加文件格式限制;
- 其他优化:
	- markdown 代码块语言识别优化; html内容未闭合标签渲染处理优化; 
	- markdown 渲染mermaid图表,AI生成部分语法错误时的兼容处理优化; 渲染失败时修正重试,将修正后的高亮代码替换显示,错误代码保留;
	- 细节优化: 主页模板可能被防火墙拦截调整：navigator.mimeTypes; 企业版默认排除第三方轻应用;卸载存储更名为删除存储
	
##### bug fixed
- 自定义滚动条区域选择文本时自动跟随滚动处理优化调整;
...
- 文件管理
	- 文件夹下内容过多时,查询处理优化(单个文件夹下文件数超过20万情况)
	- 清除已删除文件优化,避免数据过多导致查询内存溢出
	- 细节优化: 界面显示细节优化;
- 插件优化
	- 文档审批:发起审批, "指定审批人应用" 审批显示调整;
	- office阅读器: pptx读取对象存储时兼容调整;
	- AI助手插件更新,多模态解析支持(图片/视频/音频),rag解析OCR支持(加入 MinerU,paddleOCR,textin等服务支持);
- 其他优化:
	- 部门管理员角色,允许编辑为拥有者部门的部门公共标签;
	- 登录二次验证PHP兼容调整;登录二次验证第三方登录兼容调整;第三方登录信息提示优化; 存储导入插件php5.4兼容性调整
	- 文件备份频率调整;
	- 其他细节优化:用户数量获取缓存处理; 批量请求加入文件上传支持;
	
##### bug fixed
- AI助手插件,rag请求文件权限处理优化;


## 1.65 版本更新 
> **主要新增**: 细节优化 ...  
> **1.65.03 发布于: 2026.3.30**  
> **1.65.01 发布于: 2026.3.09**  

- 文件管理
	- 所有文件树目录, 增加自定义排序功能(右键设置排序模式,配置本地保存, 每个文件夹独立配置);
	- 解压缩优化调整:系统服务不存在时调用kod/bin/文件;加密文件无后缀解压异常调整;
	- 外链分享,markdown预览页面跳转自动取消锚点(知识库模式);
	- 外链分享文件夹,知识库模式时,链接引用时优化处理(url保持未编码状态; https页面引用http资源自动替换处理);
...
- 插件优化
	...
	- office阅读器xlsx单元格换行兼容调整; 默认不启用联网服务,libreoffice路径检测调整
	- autoDesk插件升级调整: 自定义配置AK/SK (由于官网sdk不再支持匿名调用,需要自行注册账号并绑定银行卡)
	- samba插件错误日志及提示优化,根部门同步异常修复;首次服务检测优化;配置文件赋权调整;
	- oauth插件更新: 微信扫码浏览器拦截兼容调整,微信扫码登录回调优化;插件配置值异常梳理;
...
- 新增CAD/3D预览插件 - autoViewer
	...
	- 插件处理
		- 插件集成: 扩展名,文件打开,文件图标处理;
		- 封面处理: 封面自动生成;文件打开封面显示;对话框打开文件生成缩略图后,自动触发父目录刷新;
		- 文件列表: 封面显示(批量查询,有封面时处理;文件变更时再次打开支持更新封面; 支持手动清除封面信息)
		- 入口及架构处理,入口引入,model方式引入js;
		- 缩略图透明处理(3d文件默认半透明; 2d文件不透明,跟随场景背景颜色; CAD缩略图默认在深色模式下生成);
		- 相对路径资源加载; 文件加全局载缓存处理(采用浏览器缓存)
		- 深色模式修改,配置本地保存(初始化主题配色处理优化); cad绘制各类图元颜色自适应浅色处理优化;
		- 文件切换自动清理处理: pcd的css3d, 动画内容等;
		- 缩略图展示优化: kod主题节目深色浅色模式缩略图显示自动优化; 缩略图显示对比度优化,使得缩小后显示更清晰;
		- 深色浅色模式切换: 切换时CAD绘制材质色彩自适应映射优化; 已生成缩略图时,打开文件显示缩略图,自适应缩略图显示处理优化;
	- 3D 解析渲染处理
		- 基于threejs自带的loader, assimpjs的wasm解析转换loader,draco,bim,web-ifc等各类3D解析loader, 实现了丰富的3d文件格式支持;
		- 3d模型预览优化: 深色模式界面底部工具栏优化;相机光照优化(增加不同角度光源); 选中材质优化(材质光照,阴影);切换正交视图显示优化;
		- 全局引入,贴图资源加载器 mtl/tga/dds loader;(暂不处理)
		- babylon 文件默认材质优化处理; 顶点颜色加入处理;
		- off 文件解析优化: 允许空行,允许多行注释,允许行尾注释,\r\n换行; 支持顶点颜色解析;
		- usdz 文件解析优化: 支持usdc解析(usdz内部usd,usdc解析);
		- plt 2D点阵图形解析渲染处理;
		- gltf部分文件打开报错处理优化;		
		- 不同材质情况,点击选中高亮处理(各类材质自适应处理)
		- 点及点阵point/points计算包围盒处理, 加入mesh加入顶点;
		- 动画处理(dae,fbx)
		- mpd-ldraw(乐高积木模型),UI更新处理
		- 3d文件,xRay后再选中,取消xRay,材质没有还原问题; 测量加入限流;
		- 测距: 支持多点测距,测面积角度等;
		- 图切割支持: 轴切割, 包围盒切割;
		- 鼠标选中内容自适应优化(自适应根据相机处理射线参数);
		- 测量工具异常优化处理(两个点连线后再点击开始点报错;没有相机时使用默认相机)
		- 点击图形高亮获取处理优化; 无贴图情况选中材质优化处理;
		- wasm解析,worker中处理优化(解析缓存处理)
		- 解析缓存处理(dwg/dxf/ assimp); 字体加载缓存(文件hash; 存储到 indexdb)
		- 其他优化: 3mf部分文件内容缺失时报错处理优化(忽略错误内容,返回已解析内容);选中支持映射(mesh.add三角形,向量判断空情况)
	- CAD 图纸解析渲染处理
		- dxf/dwfx/dwf/dwt解析处理; 完整解析器+配置表解析模式;
		- dxf 优化: 加载新文件清理处理; 自适应大小处理优化(回到home优化); 中文乱码问题处理; 字体处理优化;
		- cad绘制前字体自动匹配,动态加载处理;
		- dxf图元解析扩展支持: HELIX/MLINE/TRACE/MESH/PROXY_ENTITY/ACAD_PROXY_ENTITY/TOLERANCE/REGION
		- dxf图元绘制优化支持: HELIX/MLINE/TRACE/MESH/PROXY_ENTITY/ACAD_PROXY_ENTITY
		- 代理图元完整解析绘制优化: 解析处理,中文文字编码解析优化; 绘制支持; 子图元颜色等情况处理优化;
		- dxf绘制优化: leader 箭头绘制优化; 文字绘制颜色异常情况(覆盖了);mtext文字方向处理优化;
		- 绘制 attrib 支持加入; 绘制支持加入(insert+attrib 重复绘制支持)
		- Line 虚线绘制优化: 虚线line材质shader处理(加入 lineDistance计算); 
		- mutilLeader绘制优化: 箭头尺寸大小优化;mtext 富文本绘制优化
		- text解析优化: text/attrib 文本绘制(样式支持加入;u下划线,o上划线;多行支持; 起始点固定处理)
		- 代理图元解析支持(二进制流解析处理); 绘制支持处理(layer默认采用上层的layer);
		- text图元绘制优化(对齐方式处理优化; 九个方向点, 拉伸对齐处理优化); 对齐优化;
		- shx字体渲染绘制优化(替换为新的库,对接处理); 字体编码映射处理(中日韩,bigfont)
		- mtext 富文本解析绘制优化:
			- 富文本绘制,颜色优化(优先使用文本中定义的颜色);
			- 下划线支持; 文字x方向压缩处理优化; 上/中/下划线绘制处理; 多字连续划线连续处理;
			- 字形处理优化: 加粗/斜体处理; 文字颜色指定保持处理;
			- 对齐方式优化(没指定宽度时,用绘制后的宽度)
			- 文字中空格占位处理优化,不可见字符绘制占位符(中文空格宽度优化); 行首尾空格占位优化;
			- 上下标结构支持优化;(独立解析点等数据尝试,dwg读取数据报错处理优化); 间距优化加入;
			- 文字编码检测优化; 自动识别检测编码(proxy图元文本解析处理优化)
			- mtext内部语法解析样式优化(换行保持颜色等信息; 只有分组结束后才pop/push新的属性保存入栈;)
			- 其他优化: 换行空行占位(默认行高);行间距优化(默认1.66倍); 忽略字体指定为0的情况(避免后续错误); 中文标点符号字符后间距优化;
			- 多行文本自动换行处理优化(英文时空格才做截断处理); 换行后对齐处理优化;中文段落自动换行优化,自动换行时标点默认不放在行首;
			- gdt字体处理优化(默认重定向到gtd-ic.shx); 
			- 文字绘制方向排版优化支持(从上到下排列,从下到上排列的情况);
			- mtext 没指定宽度时x方向对齐方式处理优化(内部实现对齐); 位置处理优化; 
			- mtext 绘制默认颜色处理优化(byBlock,byLayer 颜色处理); 文字中设置颜色byBlock处理优化;
			- 中文空格等空白字符占位处理优化(ttf字体,有geometry但是没有几何点的情况处理占位);
			- mtext 多行文本分栏处理支持; columnCode数据解析处理优化(解析为分栏相关配置);
			- mtext 多行文本分栏动态分栏,静态分栏处理支持;
		- mleader 绘制优化
			- mtext采用新的的绘制方式;内部mtext绘制优化(不指定宽度; 对齐点参数优化);
			- 绘制文本优化: 对齐点处理(左中右对齐,宽度处理); 文字边框启用时绘制;
			- 多重引线-子块内容绘制支持,attdef文字内容替换处理; 箭头绘制优化(mesh填充处理; 绘制到最上层)
		- 解决threejs中TextGeometry绘制部分字体封闭shape填充异常情况(swisscbo.ttf 等字体中0689abd等字符填充异常情况)
		- 字体加载流程处理优化,shx字体加载独立模块(继承自 MeshFont); MeshFont处理优化(绘制优化)
		- dwg转dxf解析处理中,兼容attrib多组70指令情况(控制显示隐藏等属性,只保留解析第一次的值)
		- 公差图元Tolerance解析绘制(mtext绘制优化, 边框位置处理, 解析文本语法优化);
		- 图元颜色处理继承处理优化;
		- entity解析全面优化; 使用配置表解析方式(各类图元测试完善;)
		- text文字图元9个点对齐方式,旋转,x压缩,x倾斜,xy镜像处理优化; 加入镜像拉伸时处理支持;
		- 全局图元颜色处理优化(颜色优先级处理,继承块/继承图层处理; 透明度颜色处理(透明值处理优化) );
		- wipeout图元绘制优化, 填充背景色模拟擦除遮挡效果;
		- Ole2frame 绘制支持优化; 二进制数据自动识别图片处理;
		- 深色模式/浅色模式切换, 各种材质自动切换颜色处理(默认为深色; 点/线/面/填充各类材质处理; 切换大于rgb > 0.98)
		- 底部工具栏增加鼠标所在图形坐标(屏幕坐标转为场景坐标);
		- dxf文件全解析,全量替换为配置表解析模式(递归解析 对象解析优化); POLYLINE 后面VERTEX追加处理;
		- 细节优化
			- 多重引线,指定了引线颜色时处理(子内容block绘制; attrib text绘制对齐优化)
			- 多重引线,指定了引线线条类型时处理(虚线等情况;)
			- attrib/attdef 中text绘制对齐优化(没有旋转情况,对齐方式严格按对齐点处理)
			- insert图元合并优化处理(多个重复情况,避免合并丢失); insert子图元复用时颜色处理优化;
			- 文件中文编码自动识别处理;
			- 默认颜色处理优化(未指定,byLayer; layer为0=>上层layer);
			- 所有线型,lineType及lineScale处理优化(线宽缩放因子处理)
			- shx字体绘制优化; 默认字体大小设置时按baseUp进行缩放(加大字体,占满字体高度);
			- text文字绘制(text/attrib等),字符间距处理优化(根据shape的宽度进行向后叠加);
			- shx字体解析绘制优化(`一`,中文引号等字符不做底部对齐处理,避免位置错误);
			- 3dface 绘制优化,处理加入边是否可见处理;
		- 部分文件报错优化处理(hatch填充bsline绘制部分数据异常情况优化; trace绘制优化; 报错提示处理优化);
		- dwg文件加载缓存处理, wasm转换缓存处理;dwg 解析优化: worker中处理;缓存处理;
		- drawSpline 绘制优化(控制点/拟合点处理优化; 支持只通过fitPoints拟合点绘制,二次三次曲线处理);
		- obj文件mtl材质自动加载,子文件加载; ms3d 等文件材质自动加载处理,选中材质处理优化; 
		- layout 显示优化;边框绘制; layout切换后深色浅色模式切换优化; 批注工具/测量工具配置调整;
		- layout 多tab显示优化;视图初始大小显示优化(初始化为layout视图大小,比屏幕小时放大到屏幕的80% (图纸坐标系)) 
		- assimp,occt-import 转换文件优化: worker中处理; 解析结果缓存处理;
		- 尺寸优化,资源引用优化,js压缩处理(CAD预览分离iconv等库)
		- 字体自动配置加入; 外链分享文件夹权限处理优化; 树目录文件图标背景颜色自适应处理;
		- 多语言处理; 进度状态内容优化; 下载,解析,渲染,字体加载,资源加载.解析进度处理(解析过程进度更新)
		- 其他细节优化:
			- line绘制时,只有两个点且完全重合时不绘制(默认绘制点);
			- LWPOLYLINE 绘制重写优化; 支持超大尺寸文件时,有线宽时绘制优化,虚线材质支持,避免数值计算精度丢失(支持加入有凸度值bulge时弧线绘制)
			- 公共函数优化: makeMesh,makeGeometry,drawApplyExtrusionDirection; 
			- layer颜色处理优化: colorIndex为0时重置到白色(index=7);
			- mleader箭头绘制优化, 颜色处理(优先使用指定的颜色 leaderLineColor);
			- 兼容优化: layout默认名称Model/MODEL支持,避免获取为空;dxf.blocks根据图元block填充,避免后续使用报错; ATTRIB解析配置优化(flag及对其方式只取第一次出现值);
			- dwg解析优化(c代码优化,部分老版本文件解析空指针问题兼容优化,wasm解析增强)
			- 全新的图形合并处理,大幅度提升渲染性能(根据图层+材质+图形类型进行归类合并; 减少drawcall)
			- 超大尺寸图纸显示优化; 采用动态中心点偏移方法,避免Matrix提前计算导致精度丢失问题(或偏离中心非常远的文字,线条等图元出现错位);
			- CAD图纸移动端缩放部分情况闪烁问题(point绘制,默认采用 LineBasicMaterial 材质替换为 PointsMaterial); 
			- 其他:字体获取支持路径字体文件提取; 移动端界面底部工具栏优化
			- 移动端框选放大/批注绘制等情况--拖拽不移动;
			- table.style 如果样式未指定字体,且设置了ttf标志时,默认使用仿宋字体;
			- 合并图元 renderOrder 层级显示处理优化(根据合并同类型图元最大的renderOrder值)
			- 字体缓存优化(shx字体解析,ttf字体解析存储),重写缓存读写,性能优化;
			- line 绘制透明度处理优化 (entity.transparency & 0x02000000 指定颜色; color = (entity.transparency & 0x0000ff) / 255 )
			- dwg部分文件解析兼容优化(code-value,组码读取code出现为字符串时NaN情况兼容,忽略该组码,进入下一行读取); parseValue 读取bool值更多可用数据优化;
		- hatch 绘制优化
			- 绘制优化: 填充颜色加入支持(单独加入背景图层); 渐变色背景加入支持(线性渐变/径向渐变/圆柱形渐变);
			- 填充图元绘制优化(采用新的绘制方式; shader处理优化,抗锯齿加入,点绘制优化,数据预处理性能优化);
			- 填充图元绘制超大尺寸优化(超大尺寸文件,偏离中心点超远的情况填充缩放显示处理;精度自动保持;缩放确保线条和点可见);
			- 多边形路径处理优化,和下一段路径连接自动处理方向(从距离近的一端进行连接,避免跳线); 噪点忽略处理(远处很短的线条)
			- 多边形路径构造几何体优化,子多边形shape hole孔洞处理优化(buildHatchGeometry)
			- hole嵌套的子路径暂不再处理; 奇偶 hole-fill;
			- 渐变色填充,渐变类型完善支持(渐变类型支持: LINEAR,CYLINDRICAL,INVCYLINDER,RADIAL,DIAMOND,HEMISPHERICAL,INVHEMISPHERICAL,CURVED,INVCURVED)
			- 动态合并后,填充材质需要还原响应缩放,以及平移基础位置的处理;
			- 绘制填充颜色时判断处理优化(小于0时填充,取drawFillBackground & 0xffffff)
		- dwg 浏览器wasm转换,存储dxf之前保存缓存;
		- CAD解析内存占用优化, 对象合并等情况及时释放变量,避免内存占用过高;
		- 手机浏览器预览,部分绘制异常问题兼容(绘制点,使用了line等材质移动端绘制闪烁问题);
		- 其他细节优化: 解析渲染过程进度及文案展示优化;
		- 合并图元,内存占用优化兼容处理(geometry 复用情况处理,引用计数处理);
- 后台管理:
	- 存储导入优化: 改为批量写入文件,增加导入历史记录; 结果显示及日志优化;内存占用优化;md5计算异常时调整;
	- 备份功能调整:增加更详细日志,过期备份删除调用异常,文件备份总量统计异常; 备份异常捕获以便及时终止任务;取消根据记录数差异判断备份结果
	- 后台面板,概览-趋势图数据统计优化;
	- 消息通知优化: 前端通知频率异常调整;修改配置时自动重启计划任务;多消息适配;增加任务状态检测;
	- 操作日志增加目录筛选;
	- 备份删除7天前数据异常调整;
	- webdav默认存储上传异常调整;登录锁定排除webdav自动登录;
	- 批量导入用户增加csv文件检测;
- 其他优化:
	- 下载文件夹,前端压缩大于4GB导致部分文件损坏问题修复(zip64支持);
	...
	- 细节优化: markdown 渲染svg图形自适应大小处理; 移动端打开文件管理对话框标题栏图标显示优化;
	- markdown解析 mermaid 优化,mermaid库升级;mermaid包含config配置时自动识别处理(未指定lang);DeepSeek等AI生成的mermaid代码,中文引号自动修正处理;
	- markdown渲染后htmlSafe处理优化,避免DOMPurify自动补全标签 (`<p><b>1</p>\n` 转为了 `<p><b>1</b></p><b>\n</b>`)
	- js 相关库更新 (abcjs,flowchat,markdown-it);
	
##### bug fixed
- 移动端touchend点击触发上层问题(eg: 点击加号触发了弹出菜单中的内容; 屏蔽右键菜单的mouseenter/mouseleave事件);


## 1.64 版本更新 
> **主要新增**: 细节更新 ...  
> **1.64.06 发布于: 2026.02.03**  
> **1.64.01 发布于: 2025.11.18**  

- 文件管理
	- 文件夹动态优化: 子内容过多时(超过20000)动态不是最新内容情况调整: 按下层最后修改时间倒叙筛选内容;再超过上限时提取下3层文件夹内容;
	- 文件夹动态展示优化: 文件夹下多个事件合并时,"在此处"文案内容优化,在[xxx]文件夹下(不是同一个parent不做合并)
	- 外链分享优化: 指定url已存在时保存提示优化(报错时不显示复制成功);
	- 文件审批优化: 发起审批, 审批人及步骤完整展示(同审批界面步骤及人员);
	- 图片打开及缩略图优化: avif,webp; 桌面壁纸设置,文件选择图片组件支持选择avif图片;
	- menubar菜单更多展示优化; 菜单栏内容和子菜单内容区分,菜单内容排在前面;
	- 知识库方式分享文件夹, 文件预览优化(统一显示在文档区域; zip文件内再打开文件则独立预览)
	- 文件管理细节优化:部分浏览器首次打开文件夹列表显示异常情况兼容;侧边栏首次展开--图标模式自适应大小;右键菜单自适应显示处理优化;
	- 文件预览嵌入组件优化: 外链分享单文件预览/嵌入文件(text/zip/...)/嵌入url文件(首次打开拦截,后续不再拦截);知识库模式(界面);
	- 细节优化:文件管理地址栏,查看文件夹子文件夹列表分组标题显示优化; 个人标签/部门标签点击切换菜单优化;其他UI细节优化;
- 插件优化
	- 项目管理插件优化: 新建项目,内部模板为空时显示官方模版;图片加载后更新列表vlist(自适应高度);任务卡片拖拽没有变化时不提交请求;
	- excalidraw 白板插件更新(编译最新版;文件隔离支持;允许被iframe;资源库加载处理优化);
	- 协作文档通知优化: 增加top5列表;前端界面优化;
	- pdf预览,iOS端打开异常调整;
	- 钉钉app内免登跳转优化;
	- 通知中心插件: 任务频率优化;
	- wps集成插件: 新增文档中台v7版本适配,优化文件历史版本;
	- cockpit优化: 网络管理菜单优化; 增加系统重置功能;存储列表检测nfs兼容调整;
	- AI助手: 切片入库优化(大小写忽略);rag召回优化,后台管理优化; 图片解析处理支持;其他细节优化;
	- html文件预览插件,外链分享单文件时页面显示异常情况兼容;
- 其他优化:
	- 文件备份优化,文件备份进度异常调整;
	- ace编辑器组件升级到 1.43.4
	- 插件列表接口报错处理优化(隔离处理,不影响其他插件使用;show_tips/show_json/think_exception 报错输出优化)
	- 后台菜单管理,支持插件顺序调整(拖拽调整顺序; 子菜单显示在后面;  开关/子菜单:设置后优先级更高--覆盖处理)
	- 上传头像格式尺寸限制优化;
	- 细节优化: tips提示组件,多个时叠放显示优化;
	- zip文件预览(单文件预览,外链分享zip单文件预览), 打开文本文件时异常情处理;
	- sqlite模式并发性能优化: 加入wal模式并发读取模式(读读-不锁定;读写-不锁定;写读-不锁定;写写-锁定);
	
##### bug fixed
- sqlite模式后台概览面板报错处理; 兼容sum if语法处理;
- 部分Android系统手机浏览器(redmi+edge),浏览器缩放小于100%时,可滚动区域无法滚动情况兼容处理;


## 1.63 版本更新 
> **主要新增**: 细节更新 ...  
> **1.63.01 发布于: 2025.10.13**  
> **1.63.02 发布于: 2025.10.23**   

- 文件管理
	- 对外联合分享,接收端文件列表优化: 文件文件夹内容超过3000条时分页优化处理;
	- 标题菜单栏自适应收缩菜单浏览器兼容优化;
	- 跨站点文件拖拽,文件名自动处理优化(减少重命名文件的步骤),确保file表中文件名有效;
- 插件优化
	- 钉钉/企微等同步: 根部门权限被覆盖异常调整
	- 通知中心: 请求频率优化、任务自启动视频;下载限制调整
	- 项目管理: 任务编辑,截图粘贴时附件自动处理(新建/编辑/移除任务自动解析处理附件)
	- AI助手:RAG管理优化;搜索测试优化(向量搜索+全文搜索混合搜索); rerank开关自适应处理;向量数据milvus表查询优化;
- 其他优化:
	- 通知中心: ftp存储检测异常调整
	- 存储可访问性检查优化:避免生成多个重命名文件
	- 系统备份调整: 拆分数据库、文件备份；增加文件还原功能。系统安装增加密码强度检查。
	- formMaker 表单字段password优化处理, 避免浏览器不全提示,避免保存提示;
	- tab分组组件自动保存最后打开tab支持优化;
	- 文件备份优化;文件备份进度异常调整;
	- 插件更新列表显示隐藏插件,不可卸载
	- 细节优化: 跟多未知文件图标显示优化;时间格式化优化;概览查询优化

##### bug fixed
- onlyoffice外链分享权限异常调整
